...
const enrolment = defineType({
  ...
  fields: [
    ...
    defineField({ name: "headline", title: "Shared headline totals", type: "object", fields: [
      ...
      defineField({ name: "totalFemale", title: "Female learners (unconfirmed)", description: "Not used in public headline statistics while the official records conflict.", type: "number", validation: (rule) => rule.custom(nonNegativeInteger) }),
      defineField({ name: "totalMale", title: "Male learners (unconfirmed)", description: "Not used in public headline statistics while the official records conflict.", type: "number", validation: (rule) => rule.custom(nonNegativeInteger) }),
    ...
    defineField({ name: "detailedRows", title: "Class and stream male/female rows (internal reference)", description: "Preserved for correction after the school confirms the conflicting source figures. Not rendered publicly.", type: "array", of: [defineArrayMember({ type: "detailedEnrolmentRow" })] }),
