    ...
    <>
      ...
      <Section className="bg-white">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_280px]">
          <div className="overflow-x-auto border border-[var(--school-border)] bg-white">
            <table className="w-full min-w-[720px] border-collapse text-left text-sm">
              ...
              <tbody>
                {enrolmentRows.map((row) => (
                  <tr key={row.className} className="border-t border-[var(--school-border)]">
                    <th scope="row" className="px-4 py-3 font-bold text-[var(--school-blue-dark)]">{row.className}</th>
                    <td className="px-4 py-3">{formatNumber(row.femaleDay)}</td>
                    <td className="px-4 py-3">{formatNumber(row.femaleBoarding)}</td>
                    <td className="px-4 py-3">{formatNumber(row.maleDay)}</td>
                    <td className="px-4 py-3">{formatNumber(row.maleBoarding)}</td>
                    <td className="px-4 py-3 font-bold">{formatNumber(row.total)}</td>
                  </tr>
                ))}
                <tr className="border-t-2 border-[var(--school-blue)] bg-[var(--school-cream)]">
                  <th scope="row" className="px-4 py-3 font-bold text-[var(--school-blue-dark)]">Total</th>
                  <td className="px-4 py-3 font-bold">{formatNumber(enrolmentTotals.femaleDay)}</td>
                  <td className="px-4 py-3 font-bold">{formatNumber(enrolmentTotals.femaleBoarding)}</td>
                  <td className="px-4 py-3 font-bold">{formatNumber(enrolmentTotals.maleDay)}</td>
                  <td className="px-4 py-3 font-bold">{formatNumber(enrolmentTotals.maleBoarding)}</td>
                  <td className="px-4 py-3 font-bold">{formatNumber(enrolmentTotals.grandTotal)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          <div className="grid content-start gap-3 border-l-4 border-[var(--school-gold)] bg-[var(--school-cream)] p-5 text-sm">
            <p className="font-bold text-[var(--school-blue-dark)]">Summary</p>
            <p>Female learners: {formatNumber(enrolmentTotals.totalFemale)}</p>
            <p>Male learners: {formatNumber(enrolmentTotals.totalMale)}</p>
            <p>Day learners: {formatNumber(enrolmentTotals.totalDay)}</p>
            <p>Boarding learners: {formatNumber(enrolmentTotals.totalBoarding)}</p>
            <p className="pt-2 text-xs font-semibold text-[var(--school-muted)]">Figures as of {enrolmentReportingDate}.</p>
          </div>
        </div>
      </Section>
    </>
